...
// Apps management
func (a *adapter) GetApps() ([]*model.App, error) {
	var apps []*model.App
	err := a.db.Find(&apps).Error
	return apps, err
}

func (a *adapter) GetApp(id int64) (*model.App, error) {
	var app model.App
...
func (a *adapter) GetAppByName(name string) (*model.App, error) {
	var app model.App
...
func (a *adapter) CreateApp(app *model.App) (int64, error) {
...
func (a *adapter) UpdateApp(app *model.App) error {
	return a.db.Model(&model.App{}).Where("id = ?", app.ID).Updates(app).Error
}

func (a *adapter) DeleteApp(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.App{}).Error
}
...
func (a *adapter) GetFlow(id int64) (*model.Flow, error) {
	var flow model.Flow
	err := a.db.Where("id = ?", id).First(&flow).Error
	if err != nil {
		return nil, err
	}
	return &flow, nil
...
func (a *adapter) CreateFlow(flow *model.Flow) (int64, error) {
	err := a.db.Create(flow).Error
	if err != nil {
		return 0, err
	}
	return flow.ID, nil
...
func (a *adapter) DeleteFlow(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.Flow{}).Error
}

// Flow nodes management
func (a *adapter) GetFlowNodes(flowID int64) ([]*model.FlowNode, error) {
	var nodes []*model.FlowNode
	err := a.db.Where("flow_id = ?", flowID).Find(&nodes).Error
	return nodes, err
}

func (a *adapter) GetFlowNode(flowID int64, nodeID string) (*model.FlowNode, error) {
	var node model.FlowNode
	err := a.db.Where("flow_id = ? AND node_id = ?", flowID, nodeID).First(&node).Error
	if err != nil {
		return nil, err
	}
	return &node, nil
...
func (a *adapter) CreateFlowNode(node *model.FlowNode) (int64, error) {
...
func (a *adapter) UpdateFlowNode(node *model.FlowNode) error {
	return a.db.Model(&model.FlowNode{}).Where("id = ?", node.ID).Updates(node).Error
}

func (a *adapter) DeleteFlowNode(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.FlowNode{}).Error
}

func (a *adapter) DeleteFlowNodesByFlowID(flowID int64) error {
	return a.db.Where("flow_id = ?", flowID).Delete(&model.FlowNode{}).Error
}

// Flow edges management
func (a *adapter) GetFlowEdges(flowID int64) ([]*model.FlowEdge, error) {
	var edges []*model.FlowEdge
	err := a.db.Where("flow_id = ?", flowID).Find(&edges).Error
	return edges, err
}

func (a *adapter) CreateFlowEdge(edge *model.FlowEdge) (int64, error) {
	err := a.db.Create(edge).Error
	if err != nil {
		return 0, err
	}
	return edge.ID, nil
...
func (a *adapter) UpdateFlowEdge(edge *model.FlowEdge) error {
	return a.db.Model(&model.FlowEdge{}).Where("id = ?", edge.ID).Updates(edge).Error
}

func (a *adapter) DeleteFlowEdge(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.FlowEdge{}).Error
}

func (a *adapter) DeleteFlowEdgesByFlowID(flowID int64) error {
	return a.db.Where("flow_id = ?", flowID).Delete(&model.FlowEdge{}).Error
}

// Executions management
func (a *adapter) GetExecutions(flowID int64, limit int) ([]*model.Execution, error) {
	var executions []*model.Execution
	builder := a.db.Where("flow_id = ?", flowID).Order("created_at DESC")
	if limit > 0 {
		builder = builder.Limit(limit)
	}
	err := builder.Find(&executions).Error
	return executions, err
}

func (a *adapter) GetExecution(executionID string) (*model.Execution, error) {
	var execution model.Execution
	err := a.db.Where("execution_id = ?", executionID).First(&execution).Error
	if err != nil {
		return nil, err
	}
...
func (a *adapter) CreateExecution(execution *model.Execution) (int64, error) {
	err := a.db.Create(execution).Error
	if err != nil {
		return 0, err
	}
...
func (a *adapter) UpdateExecution(execution *model.Execution) error {
...
func (a *adapter) GetConnections(uid types.Uid, topic string) ([]*model.Connection, error) {
	...
	err := a.db.Where("uid = ? AND topic = ?", uid.String(), topic).Find(&connections).Error
	return connections, err
}

func (a *adapter) GetConnection(id int64) (*model.Connection, error) {
	var conn model.Connection
	err := a.db.Where("id = ?", id).First(&conn).Error
	if err != nil {
		return nil, err
	}
	return &conn, nil
...
func (a *adapter) CreateConnection(conn *model.Connection) (int64, error) {
...
func (a *adapter) UpdateConnection(conn *model.Connection) error {
	return a.db.Model(&model.Connection{}).Where("id = ?", conn.ID).Updates(conn).Error
}

func (a *adapter) DeleteConnection(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.Connection{}).Error
}

// Authentications management
func (a *adapter) GetAuthentications(uid types.Uid, topic string) ([]*model.Authentication, error) {
	var auths []*model.Authentication
	err := a.db.Where("uid = ? AND topic = ?", uid.String(), topic).Find(&auths).Error
	return auths, err
}

func (a *adapter) GetAuthentication(id int64) (*model.Authentication, error) {
...
func (a *adapter) CreateAuthentication(auth *model.Authentication) (int64, error) {
...
func (a *adapter) UpdateAuthentication(auth *model.Authentication) error {
	return a.db.Model(&model.Authentication{}).Where("id = ?", auth.ID).Updates(auth).Error
}

func (a *adapter) DeleteAuthentication(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.Authentication{}).Error
}

// Rate limits management
func (a *adapter) GetRateLimits(flowID *int64, nodeID string) ([]*model.RateLimit, error) {
	var limits []*model.RateLimit
	builder := a.db
	if flowID != nil {
		builder = builder.Where("flow_id = ?", *flowID)
	}
	if nodeID != "" {
		builder = builder.Where("node_id = ?", nodeID)
	}
	err := builder.Find(&limits).Error
	return limits, err
}

func (a *adapter) CreateRateLimit(limit *model.RateLimit) (int64, error) {
	err := a.db.Create(limit).Error
	if err != nil {
		return 0, err
	}
	return limit.ID, nil
...
func (a *adapter) UpdateRateLimit(limit *model.RateLimit) error {
	return a.db.Model(&model.RateLimit{}).Where("id = ?", limit.ID).Updates(limit).Error
}

func (a *adapter) DeleteRateLimit(id int64) error {
	return a.db.Where("id = ?", id).Delete(&model.RateLimit{}).Error
}
