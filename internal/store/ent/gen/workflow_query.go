...
type WorkflowQuery struct {
	...
	withDag      *DagQuery
	withTriggers *WorkflowTriggerQuery
...
// QueryDag chains the current query on the "dag" edge.
func (_q *WorkflowQuery) QueryDag() *DagQuery {
	query := (&DagClient{config: _q.config}).Query()
...
func (_q *WorkflowQuery) QueryTriggers() *WorkflowTriggerQuery {
	query := (&WorkflowTriggerClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *WorkflowQuery) FirstID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (_q *WorkflowQuery) FirstIDX(ctx context.Context) int64 {
...
func (_q *WorkflowQuery) OnlyID(ctx context.Context) (id int64, err error) {
	var ids []int64
...
func (_q *WorkflowQuery) OnlyIDX(ctx context.Context) int64 {
...
func (_q *WorkflowQuery) IDs(ctx context.Context) (ids []int64, err error) {
...
func (_q *WorkflowQuery) IDsX(ctx context.Context) []int64 {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
// WithDag tells the query-builder to eager-load the nodes that are connected to
// the "dag" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *WorkflowQuery) WithDag(opts ...func(*DagQuery)) *WorkflowQuery {
	query := (&DagClient{config: _q.config}).Query()
...
func (_q *WorkflowQuery) WithTriggers(opts ...func(*WorkflowTriggerQuery)) *WorkflowQuery {
	query := (&WorkflowTriggerClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *WorkflowQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Workflow, error) {
	...
	if query := _q.withDag; query != nil {
		if err := _q.loadDag(ctx, query, nodes, nil,
			func(n *Workflow, e *Dag) { n.Edges.Dag = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withTriggers; query != nil {
		if err := _q.loadTriggers(ctx, query, nodes,
			func(n *Workflow) { n.Edges.Triggers = []*WorkflowTrigger{} },
			func(n *Workflow, e *WorkflowTrigger) { n.Edges.Triggers = append(n.Edges.Triggers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *WorkflowQuery) loadDag(ctx context.Context, query *DagQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *Dag)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Workflow)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
		nodeids[nodes[i].ID] = nodes[i]
	}
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(dag.FieldWorkflowID)
	}
	query.Where(predicate.Dag(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.DagColumn), fks...))
	}))
...
func (_q *WorkflowQuery) loadTriggers(ctx context.Context, query *WorkflowTriggerQuery, nodes []*Workflow, init func(*Workflow), assign func(*Workflow, *WorkflowTrigger)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Workflow)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(workflowtrigger.FieldWorkflowID)
	}
	query.Where(predicate.WorkflowTrigger(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(workflow.TriggersColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *WorkflowQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(workflow.Table, workflow.Columns, sqlgraph.NewFieldSpec(workflow.FieldID, field.TypeInt64))
	_spec.From = _q.sql
