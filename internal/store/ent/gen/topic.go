...
type Topic struct {
	...
	// TouchedAt holds the value of the "touched_at" field.
...
func (*Topic) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case topic.FieldID, topic.FieldOwner, topic.FieldState:
			values[i] = new(sql.NullInt64)
		case topic.FieldFlag, topic.FieldPlatform, topic.FieldName, topic.FieldType, topic.FieldTags:
			values[i] = new(sql.NullString)
		case topic.FieldTouchedAt, topic.FieldCreatedAt, topic.FieldUpdatedAt:
	...
	for i := range columns {
		...
		case topic.FieldTouchedAt:
			if value, ok := values[i].(*sql.NullTime); !ok {
				return fmt.Errorf("unexpected type %T for field touched_at", values[i])
			} else if value.Valid {
				_m.TouchedAt = new(time.Time)
				*_m.TouchedAt = value.Time
			}
...
func (_m *Topic) Unwrap() *Topic {
	...
	if !ok {
		panic("gen: Topic is not a transactional entity")
	}
...
func (_m *Topic) String() string {
	...
	builder.WriteString(", ")
	if v := _m.TouchedAt; v != nil {
		builder.WriteString("touched_at=")
