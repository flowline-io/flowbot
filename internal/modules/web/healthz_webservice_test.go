...
func TestCapabilityHealthOK(t *testing.T) {
	t.Parallel()
	tests := []struct {
		name string
		data any
		...
		{name: "map healthy true", data: map[string]any{"healthy": true}, want: true},
		{name: "map healthy false", data: map[string]any{"healthy": false}, want: false},
		{name: "other type", data: "ok", want: false},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			assert.Equal(t, tt.want, capabilityHealthOK(tt.data))
		})
	}
}
