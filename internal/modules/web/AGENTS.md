...
- Auth: default route auth (cookie `accessToken` + scopes). Only login/setup/logout/`csrf-token` use `route.WithNotAuth()`. Handlers still call `authenticateWeb()` (KindFull + login redirect). CSRF double-submit (`csrfToken` / `X-CSRF-Token`); helpers in `public/js/app.js`.
