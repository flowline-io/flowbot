...
func TestCommandRules_Defines(t *testing.T) {
	assert.Equal(t, "torrent clear", commandRules[0].Define)
	assert.Equal(t, "clear torrents", commandRules[0].Help)
}
...
func TestCommandRules_TokenParsing(t *testing.T) {
	tests := []struct {
		...
		{"torrent clear", "torrent clear", true},
		{"torrent clear", "torrent clear extra", false},
		{"torrent clear", "torrent", false},
	}
...
func TestCommandRules_ClearHandler(t *testing.T) {
	t.Skip("requires transmission service")

	var clearRule *command.Rule
	for i := range commandRules {
		if commandRules[i].Define == "torrent clear" {
			clearRule = &commandRules[i]
			break
		}
	}
	require.NotNil(t, clearRule)

	ctx := types.Context{Platform: "test", Topic: "test", AsUser: types.Uid("test")}
	tokens, _ := parser.ParseString("torrent clear")

	payload := clearRule.Handler(ctx, tokens)
