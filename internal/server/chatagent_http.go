...
func RegisterChatAgentRoutes(a *fiber.App, svc *chatagent.Service) {
	...
	a.Post("/chatagent/sessions/:id/media", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.uploadSessionMedia)))
	...
	a.Get("/chatagent/approval", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.getApproval)))
	a.Put("/chatagent/approval", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.putApproval)))
	a.Delete("/chatagent/approval", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.deleteApproval)))
	a.Get("/chatagent/sessions/:id/events", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.sessionEvents)))
	a.Get("/chatagent/sessions/:id/settings", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.getSessionSettings)))
	a.Put("/chatagent/sessions/:id/settings", route.Authorize(route.RequireScope(auth.ScopeChatAgentChat, chatHTTP.putSessionSettings)))
