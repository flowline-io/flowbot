...
This is an **architectural** rule (enforced by `internal/architecture` tests), not Go's `internal/` visibility rule (same-module imports are legal).

## Rules

1. `pkg/*` must not import `internal/*`, except packages on the migration allowlist in `internal/architecture`.
...
3. Domain enums used outside the store layer live in `pkg/types` (e.g. Instruct*). `ent/schema` may type-alias them.
...
See also root [AGENTS.md](../../AGENTS.md) Boundaries.
