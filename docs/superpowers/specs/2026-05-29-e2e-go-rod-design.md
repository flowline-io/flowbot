...
```
TestMain
  ├── Start PostgreSQL container + Redis container (testcontainers)
  ├── Run ent migrations on flowbot_e2e database
  ├── Build fiber.App, mount web module routes
  ├── net.Listen("tcp", "127.0.0.1:0") → dynamic port
  ...
  │   ├── TestConfigsPage → authenticated session, navigate, wait for HTMX
  ...
  │
  └── Cleanup: browser.MustClose() → app.Shutdown() → terminate containers
```
...
A fixed database `flowbot_e2e` is used (no parallel test processes in e2e). All tables are truncated in `TestMain` after migrations and before tests run.
...
// loginAsAdmin logs in and returns the authenticated page (cookie set).
...
3. Wait for `[data-testid="configs-table"]` to appear
4. If empty DB → "No configs found." text present; if seeded → rows present
...
Flow: login → click `[data-testid="configs-new"]` → fill `[data-testid="config-uid"]`, `[data-testid="config-topic"]`, `[data-testid="config-key"]`, `[data-testid="config-value"]` → click `[data-testid="config-save"]` → `MustWaitStable()` → verify new row in table.

### TestConfigUpdate

1. `seedConfig(...)` via store
2. Login → `/configs`
3. Click `[data-testid="config-edit"]` on the seeded row
4. Form replaces row with `[data-testid="config-value"]` pre-filled
5. Modify value → click `[data-testid="config-save"]`
6. Row updates with new value

### TestConfigDelete

1. `seedConfig(...)` via store
2. Login → `/configs`
3. Click `[data-testid="config-delete"]`
4. Handle `hx-confirm` dialog → `page.MustAcceptDialog()` or similar
5. `MustWaitStable()` → row removed from table
...
| No hardcoded sleep | `page.MustWaitStable()` / `page.MustWaitIdle()` — Go-rod auto-wait |
...
| Lifecycle defense | `t.Cleanup` + `defer` + leakless — no zombie Chromium processes |
...
Use Go-rod's built-in auto-wait:
- `page.MustWaitStable()` — DOM has settled, no new frames
- `page.MustWaitIdle()` — all network requests complete
- `page.MustElement("[data-testid=xxx]")` — auto-waits for element to appear
