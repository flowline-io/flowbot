...
func (d *Runtime) doRun(ctx context.Context, t *types.Task) error {
	...
	env := d.buildEnvVars(t)

	mounts, err := d.buildContainerMounts(t)
	if err != nil {
		return err
	}
	...
	resources, err := d.buildResources(t)
	if err != nil {
		return err
	}
	...
	cc := d.buildContainerConfig(t, workdir.Target, env)
	nc := d.buildNetworkConfig(t)
...
func (d *Runtime) buildEnvVars(t *types.Task) []string {
...
func (d *Runtime) buildContainerMounts(t *types.Task) ([]mount.Mount, error) {
...
func (d *Runtime) buildResources(t *types.Task) (container.Resources, error) {
...
func (d *Runtime) buildContainerConfig(t *types.Task, workdirTarget string, env []string) container.Config {
	cmd := t.CMD
	if len(cmd) == 0 {
...
func (d *Runtime) buildNetworkConfig(t *types.Task) network.NetworkingConfig {
