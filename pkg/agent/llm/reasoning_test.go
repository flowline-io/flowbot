...
func TestSupportsReasoningStream(t *testing.T) {
	tests := []struct {
		...
		{name: "deepseek-flash is reasoning", modelName: "deepseek-flash", want: true},
		{name: "legacy deepseek-v4-flash is not catalog reasoning", modelName: "deepseek-v4-flash", want: false},
...
func TestReasoningCallOptions(t *testing.T) {
	tests := []struct {
		...
		{
			name:      "deepseek flash enables stream thinking",
