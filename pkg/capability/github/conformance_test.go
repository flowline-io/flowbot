...
func newConformanceService(t *testing.T, cfg conformance.GithubConfig) Service {
	t.Helper()
	c := &fakeClient{
...
func TestGithubConformance(t *testing.T) {
	conformance.RunGithubConformance(t, func(_ *testing.T, cfg conformance.GithubConfig) conformance.GithubService {
		return newConformanceService(t, cfg)
	})
}

func cfgToUser(user *capability.ForgeUser) *provider.User {
	if user == nil {
		return nil
	}
	id := user.ID
...
func cfgToRepo(repo *capability.ForgeRepo) *provider.Repository {
	if repo == nil {
		return nil
	}
	id := repo.ID
...
func cfgToIssues(issues []*capability.ForgeIssue) []*provider.Issue {
	if issues == nil {
		return nil
	}
...
func cfgToNotifications(notifications []*capability.Notification) []*provider.Notification {
...
func cfgToReleases(releases []*capability.Release) []*provider.RepositoryRelease {
	if releases == nil {
		return nil
	}
