...
func TestAdapter_GetUser(t *testing.T) {
	...
	tests := []struct {
		name    string
		client  *fakeClient
		want    *capability.ForgeUser
		wantErr bool
	}{
		{
			name:    "success",
			client:  &fakeClient{user: testUser(1, "testuser", "test@example.com")},
			want:    &capability.ForgeUser{ID: 1, UserName: "testuser", Email: "test@example.com"},
			wantErr: false,
		},
		{
			name:    "provider error",
...
func TestAdapter_GetRepo(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			want:    &capability.ForgeRepo{ID: 100, Name: "myrepo", FullName: "owner/myrepo", Owner: "owner"},
			wantErr: false,
		},
		{
			name:   "empty owner",
...
func TestAdapter_ListIssues(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			a := NewWithClient(tt.client)
			result, err := a.ListIssues(context.Background(), tt.owner, &ListIssuesQuery{Page: capability.PageRequest{Limit: 20}})
			if tt.wantErr {
				require.Error(t, err)
				return
			}
...
func TestDecodeTestCursor(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cursor: func() string {
				c, _ := capability.EncodeCursor(conformance.CursorSecret, capability.CursorPayload{Capability: "github", Backend: "github", Strategy: "offset", ProviderCursor: "2", Limit: 20})
				return c
			}(),
		},
		{
...
func decodeTestCursor(t *testing.T, adapter *Adapter, cursor string) (capability.CursorPayload, error) {
	t.Helper()
	return capability.DecodeCursor(adapter.cursorSecret, cursor, adapter.now())
}
