...
func (a *Adapter) ListNotifications(ctx context.Context, q *PageQuery) (*capability.ListResult[capability.Notification], error) {
	...
	items := make([]*capability.Notification, 0, len(notifications))
	for i := range notifications {
		items = append(items, toNotification(notifications[i]))
	}
	return &capability.ListResult[capability.Notification]{
		Items: items,
		Page:  &capability.PageInfo{Limit: normalizedLimit(q.Page.Limit), HasMore: false},
	}, nil
}
...
func (a *Adapter) ListReleases(ctx context.Context, owner, repo string, q *PageQuery) (*capability.ListResult[capability.Release], error) {
	...
	items := make([]*capability.Release, 0, len(releases))
	for i := range releases {
		items = append(items, toRelease(releases[i]))
	}
	result := &capability.ListResult[capability.Release]{
		Items: items,
		Page: &capability.PageInfo{
			Limit:   limit,
...
func toForgeUser(user *provider.User) *capability.ForgeUser {
	if user == nil {
		return nil
	}
...
func toForgeRepo(repo *provider.Repository) *capability.ForgeRepo {
	if repo == nil {
		return nil
	}
	ownerName := ""
...
func toForgeIssue(issue *provider.Issue) *capability.ForgeIssue {
	if issue == nil {
		return nil
	}
	author := ""
...
func toNotification(n *provider.Notification) *capability.Notification {
	if n == nil {
		return nil
	}
	repoName := ""
...
func toRelease(r *provider.RepositoryRelease) *capability.Release {
	if r == nil {
		return nil
	}
