...
func (a *Adapter) ListTasks(ctx context.Context, q *TaskQuery) (*capability.ListResult[capability.Task], error) {
	...
	items := make([]*capability.Task, 0, len(tasks))
	for _, t := range tasks {
...
func (a *Adapter) SearchTasks(ctx context.Context, q *SearchQuery) (*capability.ListResult[capability.Task], error) {
	if err := a.checkClient(); err != nil {
		return nil, err
	}
	if err := ctx.Err(); err != nil {
...
func paginateSlice[T any](items []*T, pageReq capability.PageRequest, cursorSecret []byte, now time.Time, capName, _ string) *capability.ListResult[T] {
	offset := 0
	limit := normalizedLimit(pageReq.Limit)
	if pageReq.Cursor != "" {
		if payload, err := capability.DecodeCursor(cursorSecret, pageReq.Cursor, now); err == nil {
	...
	if offset >= len(items) {
		return &capability.ListResult[T]{Items: []*T{}, Page: &capability.PageInfo{Limit: limit}}
	}
	end := min(offset+limit, len(items))
	slice := items[offset:end]
	hasMore := end < len(items)
	page := &capability.PageInfo{Limit: limit, HasMore: hasMore}
	if hasMore {
		nextCursor, err := capability.EncodeCursor(cursorSecret, capability.CursorPayload{
			Capability: capName,
			Strategy:   "offset",
			Offset:     end,
			Limit:      limit,
	...
	return &capability.ListResult[T]{Items: slice, Page: page}
}
