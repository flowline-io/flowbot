...
func TestGiteaForgeConformance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			conformance.RunForgeConformance(t, func(_ *testing.T, cfg conformance.ForgeConfig) conformance.ForgeService {
				c := &fakeClient{
...
func cfgToSDKUser(user *capability.ForgeUser) *giteasdk.User {
	if user == nil {
		return nil
	}
...
func cfgToSDKRepo(repo *capability.ForgeRepo) *giteasdk.Repository {
	if repo == nil {
		return nil
	}
...
func cfgToSDKIssues(issues []*capability.ForgeIssue) []*giteasdk.Issue {
	if issues == nil {
		return nil
	}
...
func cfgToDiff(diff *capability.ForgeCommitDiff) *provider.CommitDiff {
	if diff == nil {
		return nil
	}
...
func TestDecodeTestCursor(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "page 1", capability: "gitea", backend: provider.ID, strategy: "offset", providerCursor: "1", limit: 50},
		{name: "page 5", capability: "gitea", backend: provider.ID, strategy: "offset", providerCursor: "5", limit: 20},
		{name: "different limit", capability: "gitea", backend: provider.ID, strategy: "offset", providerCursor: "10", limit: 100},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
