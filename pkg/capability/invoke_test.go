...
func TestNewRegistry(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			r.mu.RLock()
			assert.NotContains(t, r.handlers, hub.CapKarakeep)
			r.mu.RUnlock()
		})
	}
}
...
func TestRegistry_Register(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.Register(hub.CapKarakeep, "list", invoker)
			...
			if tt.name == "registering duplicate operation overwrites previous invoker" {
				...
				err = r.Register(hub.CapKarakeep, "list", newInvoker)
				require.NoError(t, err)
			}
			if tt.name == "registering under different capability creates separate entry" {
				err = r.Register(hub.CapExample, "add", invoker)
				require.NoError(t, err)
			}
			r.mu.RLock()
			require.Contains(t, r.handlers, hub.CapKarakeep)
			require.Contains(t, r.handlers[hub.CapKarakeep], "list")
			r.mu.RUnlock()
		})
	}
}
...
func TestRegistry_RegisterEmptyCapability(t *testing.T) {
	...
	tests := []struct {
		...
		{"valid capability with empty operation returns invalid argument error", hub.CapKarakeep, "operation is required"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
...
func TestRegistry_RegisterEmptyOperation(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.Register(hub.CapKarakeep, tt.operation, invoker)
			if tt.errMsg != "" {
				require.Error(t, err)
				require.ErrorIs(t, err, types.ErrInvalidArgument)
				assert.Contains(t, err.Error(), tt.errMsg)
...
func TestRegistry_RegisterNilInvoker(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil invoker returns invalid argument error", hub.CapKarakeep, "list"},
		{"nil invoker for reader capability returns invalid argument error", hub.CapMiniflux, "list_feeds"},
		{"nil invoker for example capability returns invalid argument error", hub.CapExample, "add"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "registers operations across different capabilities" {
				require.NoError(t, r.Register(hub.CapKarakeep, "list", invoker))
				require.NoError(t, r.Register(hub.CapExample, "add", invoker))
				r.mu.RLock()
				assert.Contains(t, r.handlers, hub.CapKarakeep)
				assert.Contains(t, r.handlers, hub.CapExample)
				r.mu.RUnlock()
				return
			}
			require.NoError(t, r.Register(hub.CapKarakeep, "list", invoker))
			require.NoError(t, r.Register(hub.CapKarakeep, "get", invoker))
			require.NoError(t, r.Register(hub.CapKarakeep, "create", invoker))
			...
			r.mu.RLock()
			assert.Len(t, r.handlers[hub.CapKarakeep], 3)
			r.mu.RUnlock()
		})
	}
}
...
func TestRegistry_InvokeSuccess(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := r.Register(hub.CapKarakeep, "list", func(_ context.Context, params map[string]any) (*InvokeResult, error) {
				assert.Equal(t, "val", params["key"])
				return expected, nil
			})
			require.NoError(t, err)
			result, err := r.Invoke(t.Context(), hub.CapKarakeep, "list", map[string]any{"key": "val"})
			require.NoError(t, err)
			require.NotNil(t, result)
			assert.Equal(t, hub.CapKarakeep, result.Capability)
...
func TestRegistry_InvokeCapabilityNotFound(t *testing.T) {
	...
	tests := []struct {
		...
		{"invoke returns not found error for missing capability", hub.CapKarakeep},
		{"invoke with empty capability returns not found error", ""},
		{"invoke with unregistered capability returns not found error", hub.CapKanboard},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
...
func TestRegistry_InvokeOperationNotFound(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			r := NewRegistry()
			err := r.Register(hub.CapKarakeep, "list", func(_ context.Context, _ map[string]any) (*InvokeResult, error) {
				return &InvokeResult{}, nil
			})
			require.NoError(t, err)

			result, err := r.Invoke(t.Context(), hub.CapKarakeep, tt.operation, nil)
			require.Error(t, err)
			assert.Nil(t, result)
...
func TestRegistry_InvokeNilResultBecomesEmpty(t *testing.T) {
	...
	tests := []struct {
		...
		{"nil result from handler is replaced with empty result", hub.CapKarakeep, "list"},
		{"nil result for example capability populates correct capability/operation", hub.CapExample, "add"},
		{"nil result for kanban capability populates correct capability/operation", hub.CapKanboard, "list_tasks"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
...
func TestRegisterInvoker(t *testing.T) {
	tests := []struct {
		...
		{"convenience function registers and invokes successfully", hub.CapKarakeep, "test_op", func(_ context.Context, _ map[string]any) (*InvokeResult, error) {
		...
		{"convenience function with nil invoker returns error", hub.CapKarakeep, "list", nil, true},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
...
func TestRegistry_InvokeResultHasCapabilityAndOperation(t *testing.T) {
	...
	tests := []struct {
		...
		{"result contains capability and operation from invoke call", hub.CapExample, "add", "archived"},
		{"result contains correct capability and operation for kanban", hub.CapKanboard, "list_tasks", "tasks"},
		{"result contains correct capability and operation for reader", hub.CapMiniflux, "list_entries", "entries"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			assert.NotPanics(t, func() {
				...
				SetMetricsCollector(metrics.NewCapabilityCollector(nil))
			})
		})
	}
}
...
func TestRegistry_InvokeCacheMiss(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := r.Invoke(t.Context(), hub.CapKarakeep, "list", map[string]any{"key": "first"})
			if tt.name == "handler error is not cached" {
				require.Error(t, err)
				require.Equal(t, 1, callCount)
				_, err = r.Invoke(t.Context(), hub.CapKarakeep, "list", map[string]any{"key": "first"})
				require.Error(t, err)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "write on one capability does not affect another capability" {
				err = r.Register(hub.CapKanboard, "list_tasks", func(_ context.Context, _ map[string]any) (*InvokeResult, error) {
					kanbanCallCount++
			...
			createCallCount := 0
			err = r.Register(hub.CapKarakeep, "create", func(_ context.Context, _ map[string]any) (*InvokeResult, error) {
				createCallCount++
