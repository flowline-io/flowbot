...
func (a *Adapter) List(ctx context.Context, q *ListQuery) (*capability.ListResult[capability.Note], error) {
	...
	items := make([]*capability.Note, len(resp.Results))
	for i, n := range resp.Results {
		items[i] = toNote(&n)
	}
	return &capability.ListResult[capability.Note]{
		Items: items,
...
func (a *Adapter) Search(ctx context.Context, query string) (*capability.ListResult[capability.Note], error) {
	...
	return &capability.ListResult[capability.Note]{Items: items}, nil
}
...
// toNote maps a provider.Note to an capability.Note domain type.
func toNote(n *provider.Note) *capability.Note {
	if n == nil {
		return nil
	}
	return &capability.Note{
