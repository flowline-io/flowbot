...
func RunBookmarkConformance(t *testing.T, factory BookmarkServiceFactory) {
	t.Run("list pagination", func(t *testing.T) {
		svc := factory(t, BookmarkConfig{
			ListItems:      []*capability.Bookmark{{ID: "1", URL: "https://example.com", Title: "Example"}},
			ListNextCursor: "provider-next",
		})
		result, err := svc.List(t.Context(), &capability.BookmarkListQuery{Page: capability.PageRequest{Limit: 20}})
	...
	t.Run("list empty", func(t *testing.T) {
		svc := factory(t, BookmarkConfig{})
		result, err := svc.List(t.Context(), &capability.BookmarkListQuery{})
		require.NoError(t, err)
	...
	t.Run("list provider error", func(t *testing.T) {
		...
		_, err := svc.List(t.Context(), &capability.BookmarkListQuery{})
		RequireProviderError(t, err)
	})

	t.Run("get success", func(t *testing.T) {
		svc := factory(t, BookmarkConfig{
			GetItem: &capability.Bookmark{ID: "1", URL: "https://example.com", Title: "Test"},
		})
	...
	t.Run("create success", func(t *testing.T) {
		svc := factory(t, BookmarkConfig{
			CreateItem: &capability.Bookmark{ID: "new", URL: "https://new.example.com", Title: "New"},
		})
	...
	t.Run("search success", func(t *testing.T) {
		svc := factory(t, BookmarkConfig{
			SearchItems: []*capability.Bookmark{{ID: "1", URL: "https://example.com", Title: "Example"}},
		})
		result, err := svc.Search(t.Context(), &capability.BookmarkSearchQuery{Q: "test"})
		require.NoError(t, err)
		require.NotNil(t, result)
		require.NotNil(t, result.Items)
		require.NotNil(t, result.Page)
	...
	t.Run("search provider error", func(t *testing.T) {
		...
		_, err := svc.Search(t.Context(), &capability.BookmarkSearchQuery{})
		RequireProviderError(t, err)
	})
