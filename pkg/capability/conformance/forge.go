...
func RunForgeConformance(t *testing.T, factory ForgeServiceFactory) {
	...
	t.Run("list issues success", func(t *testing.T) {
		...
		result, err := svc.ListIssues(t.Context(), "owner", &capability.ForgeListIssuesQuery{Page: capability.PageRequest{Limit: 20}})
		require.NoError(t, err)
	...
	t.Run("list issues empty", func(t *testing.T) {
		svc := factory(t, ForgeConfig{})
		result, err := svc.ListIssues(t.Context(), "owner", &capability.ForgeListIssuesQuery{})
		require.NoError(t, err)
		require.NotNil(t, result)
		require.NotNil(t, result.Items)
	...
	t.Run("list issues provider error", func(t *testing.T) {
		...
		_, err := svc.ListIssues(t.Context(), "owner", &capability.ForgeListIssuesQuery{})
		RequireProviderError(t, err)
	})
