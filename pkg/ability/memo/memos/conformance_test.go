...
func toProviderMemo(m *ability.Memo) *provider.Memo {
	if m == nil {
		return nil
	}
	p := &provider.Memo{
...
func TestMemoConformance(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			conformance.RunMemoConformance(t, func(_ *testing.T, cfg conformance.MemoConfig) memosvc.Service {
...
func TestToProviderMemo(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			in:   &ability.Memo{Name: "memos/1", Content: "Hello", Visibility: "PUBLIC", Pinned: true, Tags: []string{"tag1"}, CreateTime: now, UpdateTime: now},
		...
		},
		{
			...
			in:   &ability.Memo{Name: "memos/1", Content: "Hi"},
