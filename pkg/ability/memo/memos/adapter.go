...
func (a *Adapter) List(ctx context.Context, q *memosvc.ListQuery) (*ability.ListResult[ability.Memo], error) {
	...
	items := make([]*ability.Memo, len(resp.Memos))
	for i, m := range resp.Memos {
		items[i] = toMemo(&m)
	}
	return &ability.ListResult[ability.Memo]{
		Items: items,
...
// toMemo maps a provider.Memo to an ability.Memo domain type.
func toMemo(m *provider.Memo) *ability.Memo {
	if m == nil {
		return nil
	}
	memo := &ability.Memo{
