...
func TestListInvokeResult_NonNil(t *testing.T) {
	...
	tests := []struct {
		name string
		lr   *ability.ListResult[ability.Bookmark]
	}{
		{"non-nil list result preserves items and page", &ability.ListResult[ability.Bookmark]{
			Items: []*ability.Bookmark{{ID: "1", URL: "https://x.com"}},
			Page:  &ability.PageInfo{Limit: 10, HasMore: false},
		}},
		{"non-nil list with multiple items preserves all", &ability.ListResult[ability.Bookmark]{
			Items: []*ability.Bookmark{
				{ID: "1", URL: "https://a.com"},
				{ID: "2", URL: "https://b.com"},
			},
			Page: &ability.PageInfo{Limit: 5, HasMore: true},
		}},
		{"non-nil list with has_more true preserves page fields", &ability.ListResult[ability.Bookmark]{
			Items: []*ability.Bookmark{{ID: "99", URL: "https://z.com"}},
			Page:  &ability.PageInfo{Limit: 50, HasMore: true, NextCursor: "cursor_next"},
		}},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
