...
func TestCreateTaskReturnsAbilityTask(t *testing.T) {
	...
	tests := []struct {
		name    string
		request kb.CreateTaskRequest
		fakeID  int64
	}{
		{"create task returns ability task with assigned id", kb.CreateTaskRequest{
			Title: "New Task", ProjectID: 1, Tags: []string{"go"},
		}, 10},
		{"create task with no tags creates task correctly", kb.CreateTaskRequest{
			Title: "Untagged", ProjectID: 2,
		}, 20},
		{"create task with multiple tags converted correctly", kb.CreateTaskRequest{
...
func TestToAbilityTaskNil(t *testing.T) {
	...
	tests := []struct {
		name string
		task *provider.Task
		want func(*testing.T, *ability.Task)
	}{
		{"nil provider task returns nil", nil, func(t *testing.T, task *ability.Task) {
			assert.Nil(t, task)
		}},
		{"non-nil empty provider task returns ability task with zero values", &provider.Task{}, func(t *testing.T, task *ability.Task) {
			require.NotNil(t, task)
			assert.Zero(t, task.ID)
			assert.Empty(t, task.Title)
		}},
		{"provider task with zero id returns zero id", &provider.Task{ID: 0, Title: "Zero"}, func(t *testing.T, task *ability.Task) {
			require.NotNil(t, task)
			assert.Zero(t, task.ID)
