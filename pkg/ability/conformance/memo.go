...
import (
	...
	mm "github.com/flowline-io/flowbot/pkg/ability/memo"
)
...
func RunMemoConformance(t *testing.T, factory MemoServiceFactory) {
	t.Run("list success", func(t *testing.T) {
		...
		result, err := svc.List(t.Context(), &mm.ListQuery{Page: ability.PageRequest{Limit: 20}})
	...
	t.Run("list empty", func(t *testing.T) {
		svc := factory(t, MemoConfig{})
		result, err := svc.List(t.Context(), &mm.ListQuery{})
		require.NoError(t, err)
	...
	t.Run("list provider error", func(t *testing.T) {
		...
		_, err := svc.List(t.Context(), &mm.ListQuery{})
		RequireProviderError(t, err)
	})

	t.Run("get success", func(t *testing.T) {
		svc := factory(t, MemoConfig{
			GetItem: &ability.Memo{Name: "memos/1", Content: "Hello"},
		})
	...
	t.Run("create success", func(t *testing.T) {
		svc := factory(t, MemoConfig{
			CreateItem: &ability.Memo{Name: "memos/1", Content: "New Memo", Visibility: "PRIVATE"},
		})
	...
	t.Run("update success", func(t *testing.T) {
		svc := factory(t, MemoConfig{
			UpdateItem: &ability.Memo{Name: "memos/1", Content: "Updated", Pinned: true},
		})
