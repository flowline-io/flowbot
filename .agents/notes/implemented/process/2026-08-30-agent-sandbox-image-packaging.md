...
- After apt (including NodeSource `nodejs`): delete `/usr/share/man`, `/usr/share/doc`, `/usr/share/info`, apt lists and archives. Keep locale data and `tzdata`.
- After `npm install -g` / `npx`: `npm cache clean --force`, remove `/root/.npm` or `/home/agent/.npm`, and delete `/tmp` contents in that same `RUN`.
...
User-facing contract stays in [Agent Sandbox](../../../../docs/agent/agent-sandbox.md) (official Go tarball, NodeSource). Interactive browsing is [Browser tools](../../../../docs/agent/browser.md), not this image. The strip list is not part of that contract.
...
- Strip locales down to `C.UTF-8` — rejected; `LANG=C.UTF-8` plus Python make locale deletion a larger runtime risk than the remaining megabytes.
...
- Dockerfile: Go strip, apt doc removal, npm cache, `/tmp`, and `GOCACHE` deletion share the `RUN` that produced those files.
- `.github/workflows/docker-agent-sandbox.yml` smoke: existing toolchain / offline `go run` checks, plus `image_size_bytes=` from `docker image inspect` (no size `exit 1`).
