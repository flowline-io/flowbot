...
func TestKanbanConfirmCommands(t *testing.T) {
	...
	for _, tc := range confirmCommands {
		t.Run(strings.Join([]string{tc.parent, tc.leaf}, "/"), func(t *testing.T) {
			parentCmd := cmd
			if tc.parent != "kanban" {
				parentCmd = findSubcommand(cmd, tc.parent)
				require.NotNil(t, parentCmd, "parent %s not found", tc.parent)
			}
			leafCmd := findSubcommand(parentCmd, tc.leaf)
			require.NotNil(t, leafCmd, "leaf %s not found under %s", tc.leaf, tc.parent)
			require.NotNil(t, leafCmd.RunE)

			yes := leafCmd.Flags().Lookup("yes")
			require.NotNil(t, yes, "--yes flag missing on %s/%s", tc.parent, tc.leaf)
