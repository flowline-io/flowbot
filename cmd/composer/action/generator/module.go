...
const BasePath = "./internal/modules"

func ModuleAction(ctx context.Context, c *cli.Command) error {
	// args
	moduleName := c.String("name")
	rule := c.StringSlice("rule") // input,group,collect,command,condition,cron,form
	if moduleName == "" {
		return errors.New("module name args error")
	}
	...
	if os.IsNotExist(err) {
		...
		err = os.WriteFile(filePath(data.BotName, "module.go"), parseTemplate(mainTemple, data), os.ModePerm)
		if err != nil {
			return err
		}
		if data.HasCollect {
		...
		if !fileExist(data.BotName, "module.go") {
			flog.Panic("dir exist, but module.go file not exist")
		}
		if data.HasInput {
			// append
			appendFileContent(filePath(data.BotName, "module.go"), parseTemplate(inputFuncTemple, data))
		}
		if !fileExist(data.BotName, "collect.go") {
			if data.HasCollect {
				...
				// append
				appendFileContent(filePath(data.BotName, "module.go"), parseTemplate(collectFuncTemple, data))
			}
		}
		if !fileExist(data.BotName, "cron.go") {
			if data.HasCron {
				...
				// append
				appendFileContent(filePath(data.BotName, "module.go"), parseTemplate(cronFuncTemple, data))
			}
		}
		if !fileExist(data.BotName, "form.go") {
			if data.HasForm {
				...
				// append
				appendFileContent(filePath(data.BotName, "module.go"), parseTemplate(formFuncTemple, data))
			}
		}
		if !fileExist(data.BotName, "instruct.go") {
			if data.HasInstruct {
				...
				// append
				appendFileContent(filePath(data.BotName, "module.go"), parseTemplate(instructFuncTemple, data))
			}
		}
	}
