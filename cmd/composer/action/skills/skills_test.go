...
func TestMetaSpecsUseCapabilityIDs(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "email", skill: "email", wantCap: string(hub.CapEmail), wantTitle: "Email", wantCLI: "email"},
	...
	tests := []workflowSkillCheck{
		...
		},
		{
			...
			skillContains: []string{
				...
				"Helpers (closed set)",
				"`now`",
				"Never invent** a helper",
				"never invent helpers",
				`function "…" not defined`,
			},
			maxSkillNewlines: 240,
		},
		{
