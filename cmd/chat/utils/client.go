// Package utils provides shared helpers for the chat terminal client.
package utils
...
// NewClient builds an authenticated Flowbot API client from flags and local config.
func NewClient(cmd *cobra.Command) (*client.Client, error) {
	...
	if serverURL == "" {
		...
		if stored == "" {
			return nil, fmt.Errorf("server URL is required (use --server-url, FLOWBOT_SERVER_URL, or 'flowbot-cli config set server-url <url>')")
		}
		serverURL = stored
	}
