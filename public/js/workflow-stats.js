(function () {
  ...
  function initChart(canvas) {
    ...
    var statsEl = document.getElementById('workflow-chart-success-rate');
    ...
    if (type === 'line') {
      ...
      var workflow = (stats.duration_distribution || {}).workflow || [];
      canvas.chartInstance = new Chart(canvas, {
        type: 'bar',
        data: {
          labels: workflow.map(function (b) {
            return b.bucket;
          }),
          datasets: [
            {
              label: flowbotI18n('client.stats.workflow_runs', 'Workflow Runs'),
              data: workflow.map(function (b) {
                return b.count;
              }),
  ...
  function initAll() {
    ...
    document
      .querySelectorAll('#workflow-stats-container canvas[data-chart-type]')
      .forEach(initChart);
  }

  document.addEventListener('htmx:beforeSwap', function (evt) {
    if (evt.detail.target.id === 'workflow-stats-container') {
      destroyCharts(evt.detail.target);
    }
  });

  document.addEventListener('htmx:afterSettle', function (_evt) {
    var container = document.getElementById('workflow-stats-container');
